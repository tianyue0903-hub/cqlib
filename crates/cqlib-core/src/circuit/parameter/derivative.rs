...
impl ExprNode {
    ...
    ///
    /// # Mathematical Note on Singularities
    /// Some functions are not strictly differentiable across their entire domain (e.g., `|x|` at `x=0`).
    /// In such cases (like `Abs` or `Sign`), this implementation may provide a subgradient approximation
    /// (e.g. evaluating to `0`). Users should be cautious when evaluating derivatives exactly at these singularities.
    ///
    /// # Arguments
    /// * `var` - The variable name to differentiate with respect to
    ///
    /// # Returns
    /// A `Result` containing a new `ExprNode` representing the derivative of this expression, or a
    /// `DerivativeError` if the expression contains non-differentiable operations (like `Mod` with a variable divisor).
    ///
    /// # Examples
    /// ```rust
    ...
    ///
    /// let deriv = expr.derivative("x").unwrap().simplify(100);
    /// assert_eq!(deriv.to_string(), "2 * x")
    /// ```
    pub fn derivative(&self, var: &str) -> Result<ExprNode, DerivativeError> {
        match self {
            ...
            ExprNode::Symbol(name) => {
                if name == var {
                    Ok(ExprNode::Integer(1)) // Derivative of variable with respect to itself is 1
                } else {
                    Ok(ExprNode::Integer(0)) // Derivative of other variables is 0
                }
            }
            // Rule 3: d(-f)/dx = -f'
            ExprNode::Neg(inner) => Ok(ExprNode::Neg(Arc::new(inner.derivative(var)?))),

            // Rule 4: d(|f|)/dx = sign(f) * f'
            // NOTE: The absolute value function is not strictly differentiable at f=0.
            // This implementation returns 0 at the singularity (since sign(0) = 0),
            // which is a common subgradient approximation but may not be mathematically strict.
            ExprNode::Abs(inner) => {
            ...
            ExprNode::Ln(inner) => {
                let inner_deriv = inner.derivative(var)?;
                Ok(ExprNode::Div(Arc::new(inner_deriv), inner.clone()))
            }
            ...
            ExprNode::Log(arg, base) => {
                ...
                if let ExprNode::E = b.as_ref() {
                    return Ok(ExprNode::Div(Arc::new(f_prime), f.clone()));
                }
                ...
                if !b.symbols().contains(var) {
                    ...
                    Ok(ExprNode::Div(
                        Arc::new(f_prime),
                        Arc::new(ExprNode::Mul(f.clone(), ln_b)),
                    ))
                } else {
                    ...
                    // Result: Term1 - Term2
                    Ok(ExprNode::Sub(Arc::new(term1), Arc::new(term2)))
                }
            }
            ...
            ExprNode::ACos(inner) => {
                let inner_deriv = inner.derivative(var)?;
                Ok(ExprNode::Neg(Arc::new(ExprNode::Div(
                    Arc::new(inner_deriv),
                    Arc::new(ExprNode::Sqrt(Arc::new(ExprNode::Sub(
                        Arc::new(ExprNode::Integer(1)),
                        Arc::new(ExprNode::Pow(inner.clone(), Arc::new(ExprNode::Integer(2)))),
            ...
            ExprNode::Add(lhs, rhs) => Ok(ExprNode::Add(
                Arc::new(lhs.derivative(var)?),
                Arc::new(rhs.derivative(var)?),
            )),

            // Rule 16: d(f - g)/dx = f' - g'
            ExprNode::Sub(lhs, rhs) => Ok(ExprNode::Sub(
                Arc::new(lhs.derivative(var)?),
                Arc::new(rhs.derivative(var)?),
            )),

            // Rule 17: d(f * g)/dx = f' * g + f * g'
            ExprNode::Mul(lhs, rhs) => Ok(ExprNode::Add(
            ...
            ExprNode::Div(lhs, rhs) => Ok(ExprNode::Div(
            ...
            ExprNode::Mod(lhs, rhs) => {
                ...
                Err(DerivativeError::NonDifferentiable(format!(
            ...
            ExprNode::Sign(_inner) => Ok(ExprNode::Integer(0)),
        }
    }
}
