...
fn test_derivative_symbol() {
    ...
    // d(x)/dx = 1
    let dx_dx = x.derivative("x").unwrap();
    assert_eq!(dx_dx, Parameter::from(1));

    // d(y)/dx = 0
    let dy_dx = y.derivative("x").unwrap();
...
fn test_derivative_negation() {
    ...
    let d_neg_x = neg_x.derivative("x").unwrap().simplify(None);
...
fn test_derivative_add_sub() {
    ...
    let d_add = add.derivative("x").unwrap().simplify(None);
    assert_eq!(d_add, Parameter::from(1));

    // d(x - 2x)/dx = 1 - 2 = -1
    let sub = x.clone() - Parameter::from(2.0) * x.clone();
    let d_sub = sub.derivative("x").unwrap().simplify(None);
...
fn test_derivative_multiplication() {
    ...
    let d_square = square.derivative("x").unwrap().simplify(None);
...
fn test_derivative_division() {
    ...
    let d_div_const = div_const.derivative("x").unwrap().simplify(None);
    assert_eq!(d_div_const, Parameter::from(0.5));

    // d(1 / x)/dx = -1/x^2
    let inv_x = Parameter::from(1.0) / x.clone();
    let d_inv_x = inv_x.derivative("x").unwrap().simplify(None);
...
fn test_derivative_power_constructed() {
    ...
    // d(x^3)/dx = 3x^2
    let deriv = x_cubed.derivative("x").unwrap().simplify(None);
    ...
    // d(2^x)/dx = 2^x * ln(2)
    let deriv_2px = two_pow_x.derivative("x").unwrap().simplify(None);
...
fn test_derivative_trig() {
    ...
    // d(sin(x))/dx = cos(x)
    let sin_x = x.sin();
    let d_sin = sin_x.derivative("x").unwrap().simplify(None);
    assert_eq!(d_sin.to_string(), "cos(x)");

    // d(cos(x))/dx = -sin(x)
    let cos_x = x.cos();
    let d_cos = cos_x.derivative("x").unwrap().simplify(None);
    assert_eq!(d_cos.to_string(), "-sin(x)");

    // d(tan(x))/dx = sec^2(x) = 1/cos^2(x)
    let tan_x = x.tan();
    let d_tan = tan_x.derivative("x").unwrap().simplify(None);
...
fn test_derivative_inverse_trig() {
    ...
    let asin_x = x.asin();
    let d_asin = asin_x.derivative("x").unwrap().simplify(None);
    assert_eq!(d_asin.to_string(), "1 / sqrt(1 - x^2)");

    // d(acos(x))/dx = -1 / sqrt(1 - x^2)
    let acos_x = x.acos();
    let d_acos = acos_x.derivative("x").unwrap();
    assert_eq!(d_acos.to_string(), "-1 / sqrt(1 - x^2)");

    // d(atan(x))/dx = 1 / (1 + x^2)
    let atan_x = x.atan();
    let d_atan = atan_x.derivative("x").unwrap();
...
fn test_derivative_exp_ln() {
    ...
    // d(e^x)/dx = e^x
    let exp_x = x.exp();
    let d_exp = exp_x.derivative("x").unwrap().simplify(None);
    assert_eq!(d_exp.to_string(), "exp(x)");

    // d(ln(x))/dx = 1/x
    let ln_x = x.ln();
    let d_ln = ln_x.derivative("x").unwrap();
...
fn test_derivative_chain_rule() {
    ...
    let deriv = sin_x2.derivative("x").unwrap().simplify(None);
...
fn test_derivative_abs_sqrt() {
    ...
    // d(|x|)/dx = sign(x)
    let abs_x = x.abs();
    let d_abs = abs_x.derivative("x").unwrap().simplify(None);
    assert_eq!(d_abs.to_string(), "sign(x)");

    // d(sqrt(x))/dx = 1 / (2 * sqrt(x))
    let sqrt_x = x.sqrt();
    let d_sqrt = sqrt_x.derivative("x").unwrap().simplify(None);
...
fn test_log_arbitrary_base() {
    ...
    let deriv = log10_x.derivative("x").unwrap().simplify(None);
...
fn test_variable_base_log() {
    ...
    let deriv = log_x_x.derivative("x").unwrap().simplify(None);
