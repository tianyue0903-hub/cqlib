...
fn test_multi_control_logic() {
    ...
    if let Instruction::Extended(ext) = &op.instruction {
        // Here we expect an MCGate(3, X) or similar, depending on implementation details
        // Since we don't have easy public access to MCGate internals, we just check it is Extended
    } else {
...
fn test_to_gate_and_circuit_gate() {
    ...
    // 2. Convert to Gate
    let bell_gate_inst = sub.to_gate("bell", vec![]).unwrap();
    ...
    if let Instruction::Circuit(gate) = &main.data[0].instruction {
...
#[test]
fn test_to_gate_parameter_mismatch() {
    let mut sub = Circuit::new(1);
    let theta = Parameter::symbol("theta");
    sub.rx(Qubit::new(0), theta).unwrap();

    // Expects 1 parameter, provide 0
    let res = sub.to_gate("fail", vec![]);
    assert!(matches!(
        res,
