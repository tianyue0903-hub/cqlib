...
use crate::circuit::{Circuit, Instruction, MCGate, Qubit, StandardGate, circuit_to_matrix};
...
fn no_aux_decompositions_match_mcgate_semantics_exactly() {
    for num_controls in 1..=3 {
        ...
        .unwrap();
        let expected = mc_swap_matrix(total, &controls, first, second);
...
fn clean_decomposition_matches_clean_subspace_and_restores_ancillas() {
    ...
    .unwrap();
    let expected = mc_swap_matrix(7, &controls, first, second);
